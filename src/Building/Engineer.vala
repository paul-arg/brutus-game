public class Brutus.Engineer : Brutus.Building {
    private static Gdk.Pixbuf sprite;

    construct {
        if (sprite == null) {
            try {
                sprite = new Gdk.Pixbuf.from_file_at_size ("%s/sprites/engineers_post.svg".printf (Build.BRUTUSDIR), TILE_WIDTH*2, -1);
            } catch (Error e) {
                critical (e.message);
            }
        }
